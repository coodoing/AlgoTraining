...
public class BBTree<K, V> {
	 ...
	 * TODO 需要考虑并发情况下的存取。 
	 */
	private static class BBTreeNode<K, V> {
		/** 节点的项，按键非降序存放 */
		private List<Entry<K, V>> entrys;
		/** 内节点的子节点 */
		private List<BBTreeNode<K, V>> children;
		/** 是否为叶子节点 */
		...
		private BBTreeNode() {
			...
			children = new ArrayList<BBTreeNode<K, V>>();
			leaf = false;
		}

		public BBTreeNode(Comparator<K> kComparator) {
			this();
		...
		int compare(K key1, K key2) {
			return kComparator == null ? ((Comparable<K>) key1).compareTo(key2)
					: kComparator.compare(key1, key2);
		}
		...
		public SearchResult<V> searchKey(K key) {
			...
			int mid = 0;
			while (low <= high) {
				mid = (low + high) / 2; // 先这么写吧，BBTree实现中，l+h不可能溢出  
		 ...
		 * @return 给定索引对应的子节点 
		 */
		public BBTreeNode<K, V> childAt(int index) {
			if (isLeaf())
		 ...
		 * @param child - 给定的子节点 
		 */
		public void addChild(BBTreeNode<K, V> child) {
			children.add(child);
		}
		 ...
		 */
		public void insertChild(BBTreeNode<K, V> child, int index) {
			List<BBTreeNode<K, V>> newChildren = new ArrayList<BBTreeNode<K, V>>();
			int i = 0;
			for (; i < index; ++i)
	 ...
	 */
	public BBTree(Comparator<K> kComparator) {
		root = new BBTreeNode<K, V>(kComparator);
		root.setLeaf(true);
		this.kComparator = kComparator;
	}

	public BBTree(Comparator<K> kComparator, int t) {
		this(kComparator);
		this.t = t;
		minKeySize = t - 1;
		maxKeySize = 2 * t - 1;
	 ...
	 * @return 键关联的值，如果存在，否则null 
	 */
	private V search(BBTreeNode<K, V> node, K key) {
	 ...
	 */
	private void splitNode(BBTreeNode<K, V> parentNode,
			BBTreeNode<K, V> childNode, int index) {
		assert childNode.size() == maxKeySize;

		BBTreeNode<K, V> siblingNode = new BBTreeNode<K, V>(kComparator);
	 ...
	 */
	private boolean insertNotFull(BBTreeNode<K, V> node, Entry<K, V> entry) {
		...
		if (node.isLeaf()) // 如果是叶子节点，直接插入  
			...
			if (result.isExist())
				return false;
			BBTreeNode<K, V> childNode = node.childAt(result.getIndex());
			if (childNode.size() == 2 * t - 1) // 如果子节点是满节点  
			{
				 ...
				 * 否则左边。 
				 */
				if (compare(entry.getKey(), node.entryAt(result.getIndex())
						.getKey()) > 0)
	...
	public boolean insert(K key, V value) {
		...
		{
			BBTreeNode<K, V> newRoot = new BBTreeNode<K, V>(kComparator);
	 ...
	 * @return true，如果B树中不存在给定的项，否则false 
	 */
	private V putNotFull(BBTreeNode<K, V> node, Entry<K, V> entry) {
		assert node.size() < maxKeySize;
	 ...
	 * @return 如果B树中存在给定键关联的项，则返回删除的项，否则null 
	 */
	private Entry<K, V> delete(BBTreeNode<K, V> node, K key) {
		...
		if (result.isExist()) {
			...
			if (node.isLeaf())
				...
				BBTreeNode<K, V> leftChildNode = node.childAt(result.getIndex());
				if (leftChildNode.size() >= t) {
					// 使用leftChildNode中的最后一个项代替node中需要删除的项  
					node.removeEntry(result.getIndex());
					node.insertEntry(leftChildNode
							.entryAt(leftChildNode.size() - 1), result
							.getIndex());
					// 递归删除左子节点中的最后一个项  
					return delete(leftChildNode, leftChildNode.entryAt(
							leftChildNode.size() - 1).getKey());
				} else {
					// 2.b 如果节点node中后于key的子节点包含至少t个关键字  
					BBTreeNode<K, V> rightChildNode = node.childAt(result
							.getIndex() + 1);
					if (rightChildNode.size() >= t) {
						// 使用rightChildNode中的第一个项代替node中需要删除的项  
						node.removeEntry(result.getIndex());
						node.insertEntry(rightChildNode.entryAt(0), result
								.getIndex());
						// 递归删除右子节点中的第一个项  
					...
					{
						Entry<K, V> deletedEntry = node.removeEntry(result
								.getIndex());
			...
			{
				//logger.info("The key: " + key + " isn't in this BBTree.");  
				return null;
			}
			BBTreeNode<K, V> childNode = node.childAt(result.getIndex());
			...
			{
				// 先查找右边的兄弟节点  
				BBTreeNode<K, V> siblingNode = null;
				int siblingIndex = -1;
				...
				if (siblingNode != null) {
					if (siblingIndex < result.getIndex()) // 左兄弟节点满足条件  
					{
						...
						node.insertEntry(siblingNode
								.entryAt(siblingNode.size() - 1), siblingIndex);
						siblingNode.removeEntry(siblingNode.size() - 1);
						// 将左兄弟节点的最后一个孩子移到childNode  
						if (!siblingNode.isLeaf()) {
							childNode.insertChild(siblingNode
									.childAt(siblingNode.size()), 0);
					...
					} else // 右兄弟节点满足条件  
					{
						childNode.insertEntry(node.entryAt(result.getIndex()),
								childNode.size() - 1);
						node.removeEntry(result.getIndex());
						node.insertEntry(siblingNode.entryAt(0), result
								.getIndex());
				...
				} else // 3.b 如果其相邻左右节点都包含t-1个项  
				{
					...
					{
						BBTreeNode<K, V> rightSiblingNode = node.childAt(result
								.getIndex() + 1);
					...
					{
						BBTreeNode<K, V> leftSiblingNode = node.childAt(result
								.getIndex() - 1);
						childNode.insertEntry(node
								.entryAt(result.getIndex() - 1), 0);
						node.removeEntry(result.getIndex() - 1);
						node.removeChild(result.getIndex() - 1);
						for (int i = leftSiblingNode.size() - 1; i >= 0; --i)
							childNode
									.insertEntry(leftSiblingNode.entryAt(i), 0);
						if (!leftSiblingNode.isLeaf()) {
							for (int i = leftSiblingNode.size(); i >= 0; --i)
								childNode.insertChild(leftSiblingNode
										.childAt(i), 0);
						}
					}
	 ...
	 */
	public void output() {
		Queue<BBTreeNode<K, V>> queue = new LinkedList<BBTreeNode<K, V>>();
		queue.offer(root);
		while (!queue.isEmpty()) {
			BBTreeNode<K, V> node = queue.poll();
	...
	public static void main(String[] args) {
		BBTree<Integer, Integer> BBTree = new BBTree<Integer, Integer>(3);
		int[] arr = new int[] { 13, 24, 37, 90, 53 };
		int r;
