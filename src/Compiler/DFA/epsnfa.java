...
public class epsnfa{
	static int maxn = 200; //maximum number of states 状态
	...
	// this is the Optimization operation ： 即把只有一个epison的表达式省略掉
	...
	// 分治 递归算法
	// 
    int[] parse(String re, int s, int t) throws Exception
	{
		int [] st;
		int i;
	...
	// https://class.coursera.org/automata/forum/thread?thread_id=269&page=1#coursera_forum_post_1002
	// it returns true if it found the final state, or false if it did not reach the final state
	// it search for all combinations of strings of {0,1}* of length 'length' 
	// 判断从当前初始状态p在最多len步中是否可以到达终态q
	boolean test(boolean [] cur, int finalstate, int level, int len, int num)
	{
		boolean[] next=new boolean[state];// 与cur数组对应
		int i,j,k,c;
		if (level>=len)
		...
		if ((num&(1<<level))>0)
			...
			c=0;
		for (i=0; i<state; ++i)
			if (cur[i])// CL闭包，可通过epsion获得
			{
				for (j=0; j<state; ++j)
					if (g[i][c][j])
					{
						for (k=0; k<state; ++k)
							next[k]=(next[k] || closure[j][k]); // 求下一步的闭包
					}
			}
		
		boolean empty=true; //test if the state set is already empty
		for (i=0; i<state; ++i)
			if (next[i]) // 如果next = false的话，即已遍历完
				empty=false;
		if (empty)
			return false;
	...
	public String Start(String filename)
	{
		...
		try{
			...
			while ((re = br.readLine()) != null) {
				...
				calc_next(re); //括号配对
				state=0;
				int[] nfa=parse(re,0,re.length()-1); // re转化: 状态
				//calculate closure
				calc_closure();				
